...
int main(){
    ...
    for(int test=0; test<t; test++){
        ...
        cout << Hull_Points << '\n';
        ll minx=Convex_Hull[0].first,maxy=Convex_Hull[0].second,idx=0;
        for(int i=1; i<Hull_Points; i++){
            if(Convex_Hull[i].second>maxy){
                maxy=Convex_Hull[i].second;
                minx=Convex_Hull[i].first;
                idx=i;
            }
            else if(Convex_Hull[i].second==maxy){
                if(Convex_Hull[i].first<minx){
                    minx=Convex_Hull[i].first;
                    idx=i;
                }
            }
        ...
        for(int i=Hull_Points-1; i>idx; i--){
            cout << Convex_Hull[i].first << ' ' << Convex_Hull[i].second << '\n';
        }
    }    
}
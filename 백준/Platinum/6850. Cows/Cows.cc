...
int main(){
    ...
    for(int i=0; i<Hull_Points; i++){
        x.push_back(Convex_Hull[i].first);
        y.push_back(Convex_Hull[i].second);
    }
    ld x1=x[0],y1=y[0];
    for(int i=1; i<Hull_Points-1; i++){
        ld x2=x[i],y2=y[i],x3=x[i+1],y3=y[i+1];
        ans+=((x1*y2 + x2*y3 + x3*y1)-(y1*x2 + y2*x3 + y3*x1))/2.0;
    }

    cout << (ll)floor(fabs(ans)/50);
}
#include <iostream>
#include <vector>
#include <algorithm>
#include <cstdio>
#include <complex>
#include <cmath>
#include <cstring>
#include <stack>
#pragma GCC optimize("O3,unroll-loops")
#pragma GCC target("avx2")
using namespace std;
typedef long long int ll;
typedef long double ld;
typedef complex<double> cpx;
const double PI = acos(-1); 

int n;
pair <ll,ll> arr[100005];
vector <pair<ll,ll>> ttemp, Convex_Hull;

ll ccw(pair <ll,ll> a, pair <ll,ll> b, pair <ll,ll> c){
    if((a.first-b.first)*(c.second-b.second)-(a.second-b.second)*(c.first-b.first)<0){
        return 1;
    }
    else if((a.first-b.first)*(c.second-b.second)-(a.second-b.second)*(c.first-b.first)==0){
        return 0;
    }
    else{
        return -1; 
    }
}

ll distance(pair <ll,ll> a, pair <ll,ll> b){
    return (b.first-a.first)*(b.first-a.first)+(b.second-a.second)*(b.second-a.second);
}

ld Euclidean_Distance(pair <ll,ll> a, pair <ll,ll> b){
    return sqrt((b.first-a.first)*(b.first-a.first)+(b.second-a.second)*(b.second-a.second));
}

bool cmp(pair <ll,ll> a, pair <ll,ll> b){
    // X좌표가 작은 거부터
    if(a.first!=b.first){
        return a.first<b.first;
    }
    // X좌표가 같으면 Y좌표가 작은 거부터
    return a.second<b.second;
}

bool cmp2(pair <ll,ll> a, pair <ll,ll> b){
    // 세 점의 ccw값이 0이라는 건 세 점들이 모두 일직선상에 존재한다는 것. 
    // 점들이 일직선 상에 존재하면 가까운 점들을 우선적으로 처리. 
    if(ccw(arr[0],a,b)==0){
        return distance(arr[0],a)<distance(arr[0],b);
    }
    return ccw(arr[0],a,b)>0;
}

int main(){
    ios_base::sync_with_stdio(false);
    cin.tie(NULL);
    cout.tie(NULL);
    cin >> n;
    int cnt=0;
    for(int i=0; i<n; i++){
        ll xx,yy;
        char tt;
        cin >> xx >> yy >> tt;
        if(tt=='Y'){
            arr[cnt].first=xx;
            arr[cnt].second=yy;
            cnt++;
        }
    }
    sort(arr,arr+cnt,cmp);
    sort(arr+1,arr+cnt,cmp2);
    Convex_Hull.push_back(arr[0]);
    Convex_Hull.push_back(arr[1]);
    for(int i=2; i<cnt; i++){
        while(Convex_Hull.size()>=2 && ccw(Convex_Hull[Convex_Hull.size()-2],Convex_Hull[Convex_Hull.size()-1],arr[i])<0){
            ttemp.push_back(Convex_Hull.back());
            Convex_Hull.pop_back();
        }
        Convex_Hull.push_back(arr[i]);
    }
    ///cout << Hull_Points << ' ' << ttemp.size() << '\n';
    for(int i=ttemp.size()-1; i>=0; i--){
        Convex_Hull.push_back(ttemp[i]);
    }
    ll Hull_Points=Convex_Hull.size();
    cout << Hull_Points << '\n';
    for(int i=0; i<Hull_Points; i++){
        cout << Convex_Hull[i].first << ' ' << Convex_Hull[i].second << '\n';
    }
}
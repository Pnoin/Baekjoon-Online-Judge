...
int t,n,Hull_Points=2;
pair <ll,ll> Convex_Hull[200005],ans1,ans2;
...
void calipers(){
    ...
    ans1=Convex_Hull[left];
    ans2=Convex_Hull[right];
	for(int i=0; i<Hull_Points; i++){
        ...
        if(distance(Convex_Hull[left],Convex_Hull[right])>ans){
            ans=distance(Convex_Hull[left],Convex_Hull[right]);
            ans1=Convex_Hull[left];
            ans2=Convex_Hull[right];
        }
...
int main(){
    ...
    for(int test=0; test<t; test++){
        cin >> n;
        Hull_Points=2;
        memset(Convex_Hull,0,sizeof(Convex_Hull));
        arr.clear();
        for(int i=0; i<n; i++){
            ll xx,yy;
            cin >> xx >> yy;

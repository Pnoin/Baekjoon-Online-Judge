...
int main(){
    ...
    for(int i=0; i<Hull_Points-1; i++){
        total+=sqrt((Convex_Hull[i].first-Convex_Hull[i+1].first)*(Convex_Hull[i].first-Convex_Hull[i+1].first)+(Convex_Hull[i].second-Convex_Hull[i+1].second)*(Convex_Hull[i].second-Convex_Hull[i+1].second));
    }
    total+=(2.0*radius*PI);
    cout << fixed;
    cout.precision(10);
